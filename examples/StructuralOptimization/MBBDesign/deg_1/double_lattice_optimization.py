    ...
    def prepare_microstructure(self):
        ...
        if self.interfaces is None:
            ...
            if self.identifier_function_neumann is not None:
                multipatch.boundary_from_function(
                    self.identifier_function_neumann[0], mask=[5]
                )
                multipatch.boundary_from_function(
                    self.identifier_function_neumann[1], mask=[4]
                )
...
def main():
    ...
    scaling_factor_objective_function = 1 / 30107.995722065854
    scaling_factor_parameters = 10
    n_refinemenets = 0
    ...
    def identifier_function_dirichlet(x):
        return (
            x[:, 0]
            >= (tiling[0] - number_of_tiles_with_load) / tiling[0] * 5.0
            - 1e-12
        )
